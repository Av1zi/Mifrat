...
Since the Sep 2026 photo rework, one product's cover is the best photo *available*, not the first one encountered, and it ships transparent.

Choosing: every vendor image URL is expanded into the same photo's originals — the vocabulary lives in one module, because the same shot is served at several shapes (a TMS listing tile is a 228px cached render of a 1500px original at an unsuffixed URL; 1PC appends `_510` to a ~500px native file; Plonter and Ivory have a single canonical shape). Candidates are scored on URL shape, whether the URL came from a detail page or a listing tile, stock, and a committed per-category vendor priority table that a human can edit without a code deploy. The winner is deterministic, so covers only move when the data moves, and detail spiders capture the gallery alongside the main image so the chooser has something to choose from. Site furniture never wins: logos, flags, menu art, and SVG icons are vetoed before scoring. A photo download treats "the file already exists" as "done" only when it is not a smaller copy of a URL we can name better; otherwise it is upgraded in place, at most once per file, tracked by a ledger so genuinely small vendor originals are not refetched forever. Image addresses without a real filename are rejected, and identifier replacements rebuild search text and specs so names stay correct.

Serving: each cover is stored as a web-sized JPEG plus a small thumbnail under per-vendor `thumbs/` folders (a few kilobytes, so category rows load them instead of full covers). An optional matting pass renders a transparent WebP beside each JPEG — the JPEG is never replaced, because matting can chew a white-on-white product, and a per-file skip list is the escape hatch when the original is better. Pages prefer the transparent render and fall back to the JPEG, so a cover with no render simply behaves as before. The public site uses local image paths only and never hotlinks vendor hosts at runtime; missing images fall back to an initials-style placeholder, and the build fails if any public product still references a remote image.
...
Only referenced images are copied to the public folder. List rows, search hits, and menu tiles use small thumbnails with full-image fallback; product and similar-card views use larger images. Images use contain-fit on the card's own background — no white plate behind them — with a hairline border so a transparent render still reads as a framed photo, and lazy loading except the main product image, which loads eagerly. Dark mode therefore no longer shows hard white rectangles around every product. A URL sanitizer only allows local image paths; remote addresses resolve to nothing so the browser never contacts vendor hosts for images, falling back to a placeholder. Link addresses are allowlisted to safe web, local, or hash targets.
